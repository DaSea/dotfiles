...
# whether display twoline
set -g theme_display_twoline yes

set -g current_bg NONE
# triangle
# set segment_separator \uE0B0
# fire
set newline_segment_separator \uE0C0
# radius
set segment_separator \uE0B4
# os
set os_segment \uF312
...
# add: the new line color
set -q color_twoline_bg; or set color_twoline_bg white
set -q color_twoline_str; or set color_twoline_str red
# os color
...
function prompt_dir -d "Display the current directory"
  prompt_segment $color_dir_bg $color_dir_str "$folder_segment"
...
function prompt_git -d "Display the current git state"
  ...
  if command git rev-parse --is-inside-work-tree >/dev/null 2>&1
    ...
    if [ "$dirty" != "" ]
      prompt_segment $color_git_dirty_bg $color_git_dirty_str "$git_symbol  $branch $dirty"
    else
      prompt_segment $color_git_bg $color_git_str "$git_symbol  $branch $dirty"
    end
  end
end
...
function prompt_svn -d "Display the current svn state"
  ...
  if command svn info >/dev/null 2>&1
    ...
    prompt_segment $color_svn_bg $color_svn_str "$svn_symbol $branch_symbol $branch:$revision"
  end
end
...
function prompt_twoline
    # new line
    echo
    set_color -b $color_twoline_bg
    set_color $color_twoline_str
    echo -n " $time_segment "
    echo -n (date +%H:%M:%S)" "
    set_color -b normal
    set_color $color_twoline_bg
    echo -n "$newline_segment_separator   "
    set_color normal
end

function prompt_os
    # display os
    # set_color -b $color_os_bg
    # set_color $color_os_str
    # echo -n " $os_segment  "
    # set_color -b normal
    # set_color normal
    # echo -n -s "$segment_separator"
    prompt_segment $color_os_bg $color_os_str "$os_segment "
end
