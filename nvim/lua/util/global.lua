...
func.translate_input_word = function()
    vim.ui.input({prompt='Input translate word:'}, function(input)
        ...
        local finalStr = 'Translate! '..input
        vim.cmd(finalStr)
    end)
    -- 采用input更简单
    --vim.fn.inputsave()
    --local translateword = vim.fn.input("Input Word:")
    --vim.fn.inputrestore()
    --if string.len(translateword)==0 then
    --    return
    --end
    --local finalStr = 'Translate! '..translateword
    --vim.cmd(finalStr)
end
-- 和上面差不多，但是在一个浮动窗口显示
--`:TranslateW[!] [--engines=ENGINES] [--target_lang=TARGET_LANG] [--source_lang=SOURCE_LANG] [your text]`
func.translatew_input_word = function()
    vim.ui.input({prompt='Input translate word:'}, function(input)
        if type(input) == 'nil' then
            return
        end
        ...
        local finalStr = 'TranslateW! '..input
        vim.cmd(finalStr)
    end)
    ...
    --    return
    --end
    --local finalStr = 'TranslateW! '..translateword
    --vim.cmd(finalStr)
end
...
func.general_operator = function()
    vim.ui.select({ 'To UTF-8', 'To GBK', 'Add bomb' },
        { prompt = 'Select operator:' }, function(item, idx)
        if idx == 1 then
